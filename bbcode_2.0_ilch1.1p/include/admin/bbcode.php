...
//> Modulauswahl
$bbmodules = array('buttons'=>'Buttons','design'=>'Design','config'=>'Konfig','badword'=>'Badword');
if ($menu->get(1) == '') {
    $menu->set_url(1,'buttons');
}
echo '<span class="visible-xs-offcanvas"><br><br></span><div class="btn-group">';
...
switch($menu->get(1)) {
	...
	case 'buttons':
		...
		if(!isset($_POST['BB_SubmitButtons'])) {
			...
			foreach($Formate as $Attribut => $Zustand) {
				if($bool[$Attribut] == 1) {
					$tpl->set($Zustand.'On' ,"active");
					$tpl->set($Zustand.'Off' ,"");
				} else {
					$tpl->set($Zustand.'On' ,"");
					$tpl->set($Zustand.'Off' ,"active");
				}
			}
		} else {
			db_query("UPDATE
			...
			$tpl->set('Message','<div class="alert alert-success" role="alert"><i class="fa fa-check-square-o"></i> Zustände wurden erfolgreich gespeichert!</div>');
	...
	case 'design':
		...
		if(!isset($_POST['BB_SubmitDesign'])) {
			...
			$tpl->set('Message','<div class="alert alert-success" role="alert"><i class="fa fa-check-square-o"></i> Design wurde erfolgreich gespeichert!</div>');
			$tpl->set('NBSP','&nbsp;');
	...
	case 'config':
		...
		if(!isset($_POST['BB_SubmitConfig'])) {
			...
			$tpl->set('Message','<div class="alert alert-success" role="alert"><i class="fa fa-check-square-o"></i> Konfiguration wurde erfolgreich gespeichert!</div>');
			//> Video "YouTube"
	...
	case 'badword':
		...
		if(isset($_POST['BB_SubmitBadword']) && $_POST['BBCode_BadPatter'] != "" && $_POST['BBCode_BadReplace'] != "") {
			...
			if(isset($if['fcBadPatter'])) {
				...
				$tpl->set('Message','<div class="alert alert-warning" role="alert"><i class="fa fa-exclamation-triangle"></i> Badword existiert schon in der Datenbank!</div>');
			} else {
				...
				$tpl->set('msgColor','#0033FF');
				$tpl->set('Message','<div class="alert alert-success" role="alert"><i class="fa fa-check-square-o"></i> Badword wurde erfolgreich gespeichert!</div>');
			}
		}
		//> Badword Löschen!
