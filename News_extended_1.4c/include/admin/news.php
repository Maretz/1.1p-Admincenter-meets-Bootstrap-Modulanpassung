...
function tn_koms() {
    ...
    $linktxt = $now == '0' ? 'ja' : '<span class="text-danger">nein</span>';
...
if (empty ($doNoIn)) {
    ...
    while ($r = db_fetch_assoc($qry)) {
        $ar['grecht'] .= '<option id="grecht_' . $r['id'] . '" name="grecht_' . $r['id'] . '" ' .
        (($Fgrecht == ($Fgrecht | pow(2, $r['id']))) ? 'checked="checked"' : '') . '>' . $r['name'] . "</option>";
    }
    // Groups
    $ar['groups'] = '';
    $qry = db_query('SELECT id, name FROM prefix_groups ORDER BY id');
    while ($r = db_fetch_assoc($qry)) {
        $ar['groups'] .= '<option id="groups_' . $r['id'] . '" name="groups_' . $r['id'] . '" ' .
        (($Fgroups == ($Fgroups | pow(2, $r['id']))) ? 'checked="checked"' : '') . '>' . $r['name'] . "</option>";
    }
    ...
    while ($row = db_fetch_object($erg)) {
        ...
        $tpl->set_ar_out(array ('ID' => $row->news_id,
                ...
                'sperre' => $row->show >= 1 ? '<i class="fa fa-thumbs-o-up"></i>' : '<i class="fa fa-thumbs-o-down"></i>',
                'sperren' => $row->show >= 1 ? 'Sperren' : 'Freischalten',
                'title' => "Erstellt von " . get_n($row->user_id) . " ($row->news_time)" . (is_null($row->editor_id) ? '' : "\n - zuletzt bearbeitet von " . get_n($row->editor_id) . " ($row->edit_time)"),
                'topnews' => $row->news_id == $opts->topnews ? '<i rel="tooltip" title="als Topnews gesetzt" style="color:#3faa22;" class="fa fa-check-circle-o"></i>' : '<i style="color:#df2037;" rel="tooltip" title="als Topnews setzen"class="fa fa-ban"></i>',
